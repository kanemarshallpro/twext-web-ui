...
export const PublishPage: React.FC<PublishPageProps> = ({ onNavigate }) => {
  ...
  const handleLoadSample = () => {
    const sampleManifest = {
      ...
      version: '1.0.0',
      description: 'A sample Twext compiled extension with fast math calculation blocks for TurboWarp.',
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-5">
        ...
        <p className="text-xs text-zinc-600 dark:text-zinc-400 max-w-md mx-auto leading-relaxed">
          Twext requires an active author session to link submissions with your namespace and manage release permissions.
        </p>
    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-16 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h1 className="text-2xl font-bold text-zinc-900 dark:text-zinc-100">Submission Received!</h1>
          <p className="text-xs text-zinc-600 dark:text-zinc-400 max-w-md mx-auto leading-relaxed">
            {publishSuccess.message}
          </p>
          <div className="pt-2 text-xs text-zinc-500 dark:text-zinc-400">
            Target: <code className="text-zinc-800 dark:text-zinc-200 font-mono">@{publishSuccess.namespace}/{publishSuccess.id}</code>
          </div>
        </div>
        ...
        <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-900/60 p-4 rounded-[6px] text-xs text-amber-900 dark:text-amber-200 max-w-lg mx-auto flex items-start gap-2.5 text-left">
          ...
          <span>
            <strong>Review Status:</strong> Newly submitted releases undergo automatic checks and moderation before appearing in public searches. You can check its progress at any time in your dashboard.
          </span>
        </div>
      ...
      {!hasAcceptedCurrentTerms && (
        <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 rounded-[6px] p-4 text-xs text-amber-900 dark:text-amber-200 flex items-start justify-between gap-3">
          <div className="flex items-start gap-2.5">
            ...
            <div>
              <strong>Terms of Service Notice:</strong> You must accept the latest Terms of Service before publishing extensions.
            </div>
          </div>
          <button
            type="button"
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white dark:bg-[#181822] border border-zinc-200 dark:border-zinc-800 rounded-[6px] p-5 space-y-3 transition-colors">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <label className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                1. Extension Manifest (<code className="text-xs text-zinc-600 dark:text-zinc-400">manifest.json</code>)
              </label>
            </div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Paste or upload the JSON manifest generated by the Twext compiler. Must contain <code className="text-zinc-700 dark:text-zinc-300">id</code>, <code className="text-zinc-700 dark:text-zinc-300">name</code>, and <code className="text-zinc-700 dark:text-zinc-300">version</code>.
          </p>
          ...
          {parsedManifest && (
            <div className="p-3 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 rounded-[4px] text-xs space-y-1 text-emerald-900 dark:text-emerald-200">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 pt-1 font-mono text-[11px] text-emerald-800 dark:text-emerald-300">
                <div>
                  <span className="text-emerald-600 dark:text-emerald-400">ID:</span> {String(parsedManifest.id || '—')}
                </div>
                <div>
                  <span className="text-emerald-600 dark:text-emerald-400">Name:</span> {String(parsedManifest.name || '—')}
                </div>
                <div>
                  <span className="text-emerald-600 dark:text-emerald-400">Version:</span> {String(parsedManifest.version || '—')}
                </div>
              </div>
            </div>
          )}
        ...
        <div className="bg-white dark:bg-[#181822] border border-zinc-200 dark:border-zinc-800 rounded-[6px] p-5 space-y-3 transition-colors">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <label className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">
                2. Compiled Extension Code (<code className="text-xs text-zinc-600 dark:text-zinc-400">extension.js</code>)
              </label>
            </div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Paste the compiled JavaScript bundle generated by <code className="text-zinc-700 dark:text-zinc-300">twext build</code>. This code executes in the Scratch/TurboWarp runtime sandbox.
          </p>
          ...
          <div className="flex items-center justify-between text-[11px] text-zinc-500 dark:text-zinc-400">
            <span>
              Size: {codeText.length > 0 ? `${(new Blob([codeText]).size / 1024).toFixed(1)} KB` : '0 KB'}
            </span>
